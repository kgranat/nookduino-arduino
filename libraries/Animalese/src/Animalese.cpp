...
#include "AnimaleseLPC.h"


long speechFrequency = 8000;
long isrRatio = (25000/ (1000000 / speechFrequency) ) ;

//#define speechFrequency 20000 // Speech engine sample rate changed from original 8000 to 10000 for Animalese
...
//#define isrRatio (25000/ (1000000 / speechFrequency) ) // gives 200 for speechFrequency 8000 -> 40 Hz or 25 ms Sample update frequency
//#define isrRatio (5000/ (1000000 / speechFrequency) ) // gives 200 for speechFrequency 8000 -> 40 Hz or 25 ms Sample update frequency
//#define isrRatio (25000/ (1000000 / speechFrequency) ) // gives 200 for speechFrequency 8000 -> 40 Hz or 25 ms Sample update frequency
...
static Animalese *sPointerToTalkieForISR;
//#if isrRatio < 255
...
Animalese::Animalese(long userSpeechFrequency = 8000, long userSpeechPeriod = 25000){





 isrRatio = (userSpeechPeriod/ (1000000 / speechFrequency) ) ;


    speechFrequency = userSpeechFrequency;
...
Animalese::Animalese(bool aUseNonInvertedOutputPin, bool aUseInvertedOutputPin) { // @suppress("Class members should be properly initialized")
...
void Animalese::setPtr(const uint8_t * aAddress) {
 ...
 */
uint8_t Animalese::getNumberOfWords() {
#if defined(ESP8266)
...
uint8_t Animalese::rev(uint8_t a) {
// 76543210
 ...
 */
uint8_t Animalese::getBits(uint8_t bits) {
// 156 bytes compiled
    uint8_t value;
    uint16_t data;
 ...
 * Add to queue (back)
 */
void Animalese::FIFOPushBack(const uint8_t *aAddress) {
    free--;
 ...
 */
const uint8_t * Animalese::FIFOPopFront() {
// 56 bytes compiled
 ...
 * The blocking version
 */
void Animalese::say(const uint8_t * aWordDataAddress) {
 ...
 */
void Animalese::doNotUseUseInvertedOutput(bool aDoNotUseInvertedOutput) {
 ...
 */
void Animalese::doNotUseNonInvertedOutput(bool aDoNotUseNonInvertedOutput) {
 ...
 */
void Animalese::initializeHardware() {
...
#endif
    OCR1A = ((F_CPU + (speechFrequency / 2)) / speechFrequency) - 1;  // 'speechFrequency' Hz (w/rounding)
    OCR1B = ((F_CPU + (speechFrequency / 2)) / speechFrequency) - 1;  // use the same value for register B
...
#define _10_BIT_OUTPUT
    sIntervalTimer.begin(timerInterrupt, 1000000L / speechFrequency);
#else // (CORE_TEENSY)
...
void Animalese::terminateHardware() {
#if defined(__AVR__)
#if F_CPU <= 8000000
 ...
 */
int8_t Animalese::sayQ(const uint8_t * aWordDataAddress) {
